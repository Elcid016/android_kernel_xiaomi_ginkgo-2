...
export KBUILD_COMPILER_STRING="$($HOME/clang/proton/bin/clang --version | head -n 1 | perl -pe 's/\((?:http|git).*?\)//gs' | sed -e 's/  */ /g' -e 's/[[:space:]]*$//')"
...
if [ ! -d AnyKernel3 ]; then
 git clone https://github.com/HafizZiq/AnyKernel3 --depth=1 -b sixteen
fi;
